...
class DeliveryPersonBankAccountService
{
    ...
    {
        return DeliveryPersonBankAccount::orderBy('id', 'asc')->get();
    }
    ...
    public function getPaginate($data)
    {
        ...
        return DeliveryPersonBankAccount::with('deliveryPerson')
    ...
    public function validateCreateAccountData($data)
    {
        return Validator::make($data, [
            'delivery_person_id'  => 'required|uuid|exists:delivery_persons,id',
            'account_holder_name' => 'required|string|max:255',
            'account_number'      => 'required|string|max:50|unique:delivery_person_bank_accounts,account_number',
    ...
    {
        return DeliveryPersonBankAccount::with('deliveryPerson')->findOrFail($id);
    }
    ...
    public function validateUpdateAccountData($data)
    {
        return Validator::make($data, [
            ...
            'delivery_person_id'  => 'required|uuid|exists:delivery_persons,id',
            'account_holder_name' => 'required|string|max:255',
            'account_number'      => 'required|string|max:50|unique:delivery_person_bank_accounts,account_number,'.$data['id'],
            'bank_name'           => 'required|string|max:255',
            'branch_name'         => 'required|string|max:255',
            'ifsc_code'           => 'required|string|max:20',
            'account_type'        => 'required|string|in:savings,current',
    ...
    {
        $account = DeliveryPersonBankAccount::findOrFail($data['id']);
    ...
    {
        $account = DeliveryPersonBankAccount::findOrFail($id);
        $account->delete();
    ...
    {
        try {
            // Get bank account details with delivery person relationship
            $bankAccount = DeliveryPersonBankAccount::with('deliveryPerson')->findOrFail($accountId);
            ...
            $requestData = [
                ...
                'phone' => $bankAccount->deliveryPerson->phone ?? '0000000000',
            ];
            ...
            Log::error("Cashfree Bank verification failed: {$e->getMessage()}", [
