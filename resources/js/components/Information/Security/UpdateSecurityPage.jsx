...
const UpdateSecurityPage = () => {
    ...
    const { securityPageId } = useParams();
    ...
    const handleUpdate = () => {
        ...
        formData.append('id', securityPageId);
        ...
        axios.post(`${apiUrl}/admin/security_pages/update_security_page`, formData, {
            headers: {
    ...
    useEffect(() => {
        // Fetch security page data based on securityPageId
        setLoading(true);
        const token = localStorage.getItem('token');
        axios.put(
            `${apiUrl}/admin/security_pages/find_security_page`,
            qs.stringify({ id: securityPageId }),
            {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            .then((response) => {
                const securityPageData = response.data.data;
                setTitle(securityPageData.title);
                setIntroduction(securityPageData.introduction);
                setContentSections(JSON.parse(securityPageData.content_sections));
                setImagePath(securityPageData.image_path ?? "/image/no_image.webp");
                setIsActive(securityPageData.is_active);
                setLoading(false);
            })
            .catch((error) => {
                showAlert({
                    title: "Error!",
                    text: "Error fetching security page data: " + error.message,
                    icon: "error",
                });
                setLoading(false);
            });
    }, [securityPageId, apiUrl]);
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && (
                    <Grid>
                        <PageHeader title="Update Security Page" handleBackClick={handleBackClick} />
                        ...
                        <ActionButtons
                            ...
                            loading={loading}
                            submitLabel="Update Security Page"
                        />
                    </Grid>
                )}
            </Paper>
