...
const AddShopBankAccountPage = () => {
    ...
    const [shopsLoading, setShopsLoading] = useState(true); // New state for shops loading
    ...
    const handleSubmit = () => {
        ...
        axios.post(`${apiUrl}/admin/shop_bank_account/create_account`, formData, {
            headers: {
    ...
    useEffect(() => {
        const fetchShops = async () => {
            try {
                ...
                setShops(response.data.data || []); // Ensure we always set an array
            } catch (error) {
                showAlert({
                    title: "Error!",
                    text: `Error fetching shops: ${error?.response?.data?.message || error.message}`,
                    icon: "error",
                });
                setShops([]); // Set empty array on error
            } finally {
                setShopsLoading(false); // Set loading to false when done
            }
        };
...
export default AddShopBankAccountPage;
