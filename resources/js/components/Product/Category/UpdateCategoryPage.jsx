...
const UpdateCategoryPage = () => {
    ...
    const handleUpdate = () => {
        ...
        axios.post(`${apiUrl}/admin/category/update_category`, formData, {
            headers: {
    ...
    useEffect(() => {
        ...
        const token = localStorage.getItem('token');
        axios.put(
            `${apiUrl}/admin/category/find_category`,
            qs.stringify({ id: categoryId }),
            {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            .then((response) => {
                ...
                setShopName(categoryData.shop_id);
                setDescription(categoryData.description);
                setStatus(categoryData.status.toString()); // Ensure status is a string
                setContentImage(categoryData.content_image ?? "/image/no_image.webp");
                setImage(categoryData.image ?? "/image/no_image.webp");
            })
            .catch((error) => {
                showAlert({
                    title: "Error!",
                    text: "Error fetching category data:", error,
                    icon: "error",
                });
            });
    }, [categoryId, apiUrl]);
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && (
                    <Grid>
                        <PageHeader title="Update Category" handleBackClick={handleBackClick} />
