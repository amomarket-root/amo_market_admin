...
const UpdateBlogPage = () => {
    ...
    const handleUpdate = () => {
        ...
        axios.post(`${apiUrl}/admin/blog/update_blog`, formData, {
            headers: {
    ...
    useEffect(() => {
        ...
        const token = localStorage.getItem('token');
        axios.put(
            `${apiUrl}/admin/blog/find_blog`,
            qs.stringify({ id: blogId }), // Serialize the id parameter
            {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            .then((response) => {
                ...
                // Parse the other_images field from JSON string to array
                const parsedOtherImages = JSON.parse(blogData.other_images);

                // Format the date to YYYY-MM-DD
                const formattedDate = blogData.date.split('T')[0];

                setMainTitle(blogData.main_title);
                setCategory(blogData.category);
                setDate(formattedDate); // Set the formatted date
                setLocation(blogData.location);
                setCustomUrl(blogData.custom_url);
                setMultimedia(blogData.multimedia || "/image/no_image.webp");
                setHeader(blogData.header);
                setDescription(blogData.description);

                // Ensure otherImages is always an array
                setOtherImages(Array.isArray(parsedOtherImages) ? parsedOtherImages : []);
                setLoading(false);
            })
            .catch((error) => {
                showAlert({
                    title: "Error!",
                    text: "Error fetching blog data:", error,
                    icon: "error",
                });
                setLoading(false);
            });
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && (
                    <Grid>
                        <PageHeader title="Update Blog/News" handleBackClick={handleBackClick} />
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid container spacing={2} sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start" }}>
                                <Grid item xs={12}>
                                    ...
                                    <Grid container spacing={2}>
                                        {Array.isArray(otherImages) && otherImages.map((image, index) => (
