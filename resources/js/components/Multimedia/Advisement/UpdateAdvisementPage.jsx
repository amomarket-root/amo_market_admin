...
const UpdateAdvisementPage = () => {
    const navigate = useNavigate();
    const showAlert = useSweetAlert();
    const showSnackbar = useSnackbar();
    const { advisementId } = useParams();
    const [title, setTitle] = useState('');
    const [shopId, setShopId] = useState('');
    const [shops, setShops] = useState([]);
    const [description, setDescription] = useState('');
    const [status, setStatus] = useState("0"); // Default to Inactive (0)
    const [contentImage, setContentImage] = useState("/image/no_image2.png");
    ...
    const handleUpdate = () => {
        ...
        formData.append('advisement_id', advisementId);
        ...
        axios.post(`${apiUrl}/admin/advisement/update_advisement`, formData, {
            headers: {
    ...
    useEffect(() => {
        // Fetch advisement data based on advisementId
        setLoading(true);
        const token = localStorage.getItem('token');
        axios.put(
            `${apiUrl}/admin/advisement/find_advisement`,
            qs.stringify({ id: advisementId }),
            {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            .then((response) => {
                const advisementData = response.data.data;
                setTitle(advisementData.title);
                setShopId(advisementData.shop_id);
                setDescription(advisementData.description);
                setStatus(advisementData.status.toString()); // Ensure status is a string
                setContentImage(advisementData.content_image ?? "/image/no_image.webp");
                setLoading(false);
            })
            .catch((error) => {
                showAlert({
                    title: "Error!",
                    text: "Error fetching advisement data:", error,
                    icon: "error",
                });
            });
    }, [advisementId, apiUrl]);
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && (
                    <Grid>
                        <PageHeader title="Update Advisement" handleBackClick={handleBackClick} />
...
export default UpdateAdvisementPage;
