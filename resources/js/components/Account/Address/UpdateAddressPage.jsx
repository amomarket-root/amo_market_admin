...
const UpdateAddressPage = () => {
        ...
        const fetchAddress = async () => {
            ...
            try {
                ...
                const response = await axios.put(
                    `${apiUrl}/admin/address/find`,
                    qs.stringify({ id: addressId }),
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                ...
                setAlternativeNumber(addressData.alternative_number || "");
                setPinCode(addressData.pin_code);
                setState(addressData.state);
                setCity(addressData.city);
                setBuildingDetails(addressData.building_details);
                setLocation(addressData.location);
                setIsDefault(addressData.is_default.toString());
                setAddressType(addressData.address_type);
                setDeliveryNote(addressData.delivery_note || "");
                setStatus(addressData.status.toString());
                setFullAddress(addressData.full_address);
                ...
                if (addressData.user) {
                    setSelectedUser({
                        label: addressData.user.name,
                        value: addressData.user.id
                    });
                }
            } catch (error) {
                showSnackbar("Error fetching address data:", error, { severity: 'error' }, 2000);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleUpdate = () => {
        ...
        axios.post(`${apiUrl}/admin/address/update`, formData, {
            headers: {
                Authorization: `Bearer ${token}`,
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && address && (
                    <Grid>
                        <PageHeader title="Update Address" handleBackClick={handleBackClick} />
