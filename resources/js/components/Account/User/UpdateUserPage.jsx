...
const UpdateUserPage = () => {
    ...
    const handleCancelClick = () => {
        navigate("/admin/user"); // Replace '/desired/path' with the path you want to navigate to on cancel
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const [userResponse, avatarsResponse] = await Promise.all([
                    axios.put(
                        `${apiUrl}/admin/user/find_user`,
                        qs.stringify({ id: userId }),
                        {
                            headers: {
                ...
                setAvatar(userData.avatar?.path ?? "/image/no_avatar.webp");

                // Handle avatars list
                const fetchedAvatars = avatarsResponse.data.map(avatar => ({
                    id: avatar.id,
                    path: avatar.path,
                }));
                ...
                showAlert({
                    title: "Error!",
                    text: "Error fetching data:" + (error || "Please try again later."),
                    icon: "error",
                });
            } finally {
                setLoading(false);
    ...
    const handleUpdate = () => {
        ...
        axios.post(`${apiUrl}/admin/user/update_user`, formData, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        })
    ...
    return (
        <Box sx={{ display: "flex" }}>
            ...
            >
                ...
                {!loading && (
                    <Grid>
                        <PageHeader title="Update User" handleBackClick={handleBackClick} />
